...
class _TeamScreenState extends State<TeamScreen> {
  final TeamStatsProvider _statsProvider = TeamStatsProvider();
      ...
      body: Container(
        ...
        child: SafeArea(
          child: ListenableBuilder(
            listenable: _statsProvider,
            builder: (context, _) {
              if (_statsProvider.isLoading) {
                return const Center(
                  child: CircularProgressIndicator.adaptive(),
                );
              }

              if (_statsProvider.error != null) {
  ...
  Widget _buildRankingsSection(Team? userTeam) {
    final rankings = _statsProvider.rankings;
  ...
  Widget _buildPurpleBuffSection() {
    final participation = _statsProvider.purpleParticipation;
          ...
          GestureDetector(
            onTap: () {
              Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (context) => const TraitorGateScreen(),
                ),
              );
            },
            child: Container(
