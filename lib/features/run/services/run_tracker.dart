...
import 'running_score_service.dart';
import 'accelerometer_service.dart';
import 'lap_service.dart';
...
class RunTracker {
  ...
  void _emitCheckpoint() {
    ...
    // Serialize frozen config snapshot for crash recovery (ensures recovered
    // run uses the same config values as the original run session)
    String? configSnapshotJson;
    try {
      configSnapshotJson = jsonEncode(
        RemoteConfigService().configSnapshot.toJson(),
      );
    } catch (e) {
      debugPrint('RunTracker: Failed to serialize config snapshot: $e');
    }
