...
class _RouteMapState extends State<RouteMap> with TickerProviderStateMixin {
  ...
  bool _isProcessingRouteUpdate = false; // Prevent overlapping updates
  ...
  void _onCameraChange(CameraChangedEventData event) {
    ...
    // Increment counter to trigger marker position update
    setState(() {
      _cameraChangeCounter++;
    });

    // Only redraw hexagons for manual camera changes in non-navigation mode
    if (widget.showHexGrid && _isMapReady && !widget.navigationMode) {
  ...
  void didUpdateWidget(RouteMap oldWidget) {
    ...
    debugPrint(
      'RouteMap didUpdateWidget: teamColor old=${oldWidget.teamColor}, new=${widget.teamColor}',
    );
    if (widget.teamColor != oldWidget.teamColor && widget.route.isNotEmpty) {
      debugPrint('RouteMap: teamColor CHANGED - calling _drawRoute');
    ...
    debugPrint(
      'RouteMap didUpdateWidget: oldVersion=${oldWidget.routeVersion}, newVersion=${widget.routeVersion}',
    );
    if (widget.routeVersion != oldWidget.routeVersion) {
      debugPrint(
        'RouteMap: routeVersion CHANGED ${oldWidget.routeVersion} -> ${widget.routeVersion}, '
        'navMode=${widget.navigationMode}, routeLen=${widget.route.length}',
      );
      _processRouteUpdate();
    } else {
      debugPrint('RouteMap: routeVersion UNCHANGED (${widget.routeVersion})');
    }
  ...
  Future<void> _processRouteUpdate() async {
    debugPrint('>>> _processRouteUpdate CALLED <<<');
    // Guard conditions - check mounted first to avoid setState after dispose
    if (!mounted) {
      debugPrint('>>> _processRouteUpdate: NOT MOUNTED, returning <<<');
      return;
    }
    if (!_isMapReady) {
      debugPrint('RouteMap: Skipping update - map not ready');
      return;
    }
    if (_mapboxMap == null) {
      debugPrint('RouteMap: Skipping update - mapboxMap is null');
      return;
    }
    if (widget.route.isEmpty) {
      debugPrint('RouteMap: Skipping update - route is empty');
      return;
    }
    ...
    if (_isProcessingRouteUpdate) {
      debugPrint(
        'RouteMap: Overlapping update v${widget.routeVersion} - skipping',
      );
      return;
    }
    ...
    debugPrint(
      'RouteMap: Processing update v${widget.routeVersion}, ${widget.route.length} points, '
      'navMode=${widget.navigationMode}, running=${widget.isRunning}',
    );

    try {
      // 1. Update camera (navigation mode only) - synchronous, non-blocking
      debugPrint(
        'RouteMap: navMode=${widget.navigationMode}, routeLen=${widget.route.length}',
      );
      if (widget.navigationMode && widget.route.length >= 2) {
        debugPrint('RouteMap: CALLING _updateNavigationCamera');
        _updateNavigationCamera(); // Don't await - fire and forget
      } else {
        debugPrint(
          'RouteMap: SKIPPING camera update - navMode=${widget.navigationMode}, len=${widget.route.length}',
        );
      }

      // 2. Update route line (the tracing path)
      if (mounted) await _drawRoute();

      // 3. Update hexagons (throttled internally)
      if (mounted && widget.showHexGrid) {
        await _drawHexagons();
      }

      // 4. Force widget rebuild for marker update
      if (mounted) {
        setState(() {
          _cameraChangeCounter++;
        });
  ...
  void _updateNavigationCamera() {
    if (_mapboxMap == null || widget.route.length < 2) {
      debugPrint(
        'NavCamera: SKIP - mapbox=${_mapboxMap != null}, routeLen=${widget.route.length}',
      );
      return;
    }
  ...
  Future<void> _drawRoute({bool forceRedraw = false}) async {
    debugPrint('>>> _drawRoute CALLED (forceRedraw=$forceRedraw) <<<');
    if (_mapboxMap == null ||
        widget.route.isEmpty ||
        _polylineManager == null) {
      debugPrint(
        'RouteMap._drawRoute: Skipping - mapbox=${_mapboxMap != null}, '
        'route=${widget.route.length}, polylineManager=${_polylineManager != null}',
      );
      return;
    }
    ...
    try {
      ...
      if (validPoints.isEmpty) {
        debugPrint(
          'RouteMap: No valid points to draw (total: ${widget.route.length})',
        );
        return;
      }
      ...
      final needsUpdate =
          ...
          forceRedraw ||
          hasNewPoints ||
          isActiveRun; // Always update during active run

      debugPrint(
        'RouteMap._drawRoute: needsUpdate=$needsUpdate, hasNewPoints=$hasNewPoints, '
        'isActiveRun=$isActiveRun, routeLen=${widget.route.length}, coords=${coordinates.length}',
      );
      ...
      if (needsUpdate && coordinates.length >= 2) {
        ...
        if (_routeAnnotation != null && !shouldRecreate) {
          try {
            ...
            debugPrint(
              'RouteMap: Updated route line to ${coordinates.length} points',
            );
            return; // Success - exit early
          } catch (e) {
            // Update failed - will recreate below
            debugPrint('RouteMap: Update failed ($e), will recreate');
            _routeAnnotation = null;
          }
        }
        ...
        try {
          ...
          debugPrint('RouteMap: deleteAll failed: $e');
        }
        _routeAnnotation = null;
        ...
        debugPrint(
          'RouteMap: Created route line with ${coordinates.length} points, width=$lineWidth',
        );
        ...
        _routeAnnotation = null;
        _lastRouteLength = widget.route.length;
        debugPrint('RouteMap: Single point, waiting for more points');
      }
    } catch (e) {
  ...
  Future<void> _drawHexagonsAround(latlong.LatLng center) async {
    ...
    try {
      ...
      for (int i = 0; i < hexIds.length; i++) {
        ...
        final hex = HexDataProvider().getHex(hexId, hexCenter);
