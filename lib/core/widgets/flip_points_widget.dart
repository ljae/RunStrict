...
class FlipPointsWidget extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<FlipPointsWidget> createState() => _FlipPointsWidgetState();
}

class _FlipPointsWidgetState extends ConsumerState<FlipPointsWidget>
  ...
  void initState() {
    super.initState();
    _displayedPoints = ref.read(pointsProvider.notifier).totalSeasonPoints;
  ...
  void _checkPendingPoints() {
    final newPoints = ref.read(pointsProvider.notifier).totalSeasonPoints;
  ...
  void _animateToPoints(int newPoints) {
    ...
    Future.delayed(Duration(milliseconds: delay + 400), () {
      if (mounted) {
        ...
        // Check if more points arrived during animation
        final latestPoints = ref.read(pointsProvider.notifier).totalSeasonPoints;
  ...
  Widget build(BuildContext context) {
    // Watch points state to trigger rebuilds on change
    ref.listen<PointsState>(pointsProvider, (previous, next) {
      _onPointsChanged();
    });
  ...
  Widget build(BuildContext context) {
    return Row(
      ...
      children: [
        ...
        const SizedBox(width: 4),
        Text(
          PointsNotifier.formatPoints(points),
