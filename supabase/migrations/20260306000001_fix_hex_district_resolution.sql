-- ============================================================================
-- FIX: H3 Resolution Mismatch in Hex District Queries
-- ============================================================================
-- Root cause: hexes.parent_hex stores Res-5 (province) IDs.
-- calculate_daily_buffs() and get_user_buff() fallback both query
--   WHERE h.parent_hex = v_city_hex  -- v_city_hex is Res-6 (district) → always 0 rows
-- Fix: Add district_hex (Res-6) column to hexes table, populated by finalize_run,
--      and use it in per-district buff queries.
-- ============================================================================

-- ============================================================================
-- STEP 1: Add district_hex column to hexes table
-- ============================================================================

ALTER TABLE public.hexes
  ADD COLUMN IF NOT EXISTS district_hex TEXT;

-- Index for fast per-district buff queries
CREATE INDEX IF NOT EXISTS idx_hexes_district_hex
  ON public.hexes (district_hex)
  WHERE last_runner_team IS NOT NULL;

-- ============================================================================
-- STEP 2: Backfill district_hex from parent_hex where possible.
-- We can't compute H3 res-6 parent without the H3 extension, so we backfill
-- from hex_snapshot which already has parent_hex (res-5). For existing rows
-- we can't derive res-6 without H3. New rows will be populated by finalize_run.
-- NOTE: This migration leaves existing rows NULL; they will be filled as users
-- run. The daily buff cron falls back gracefully (0 hex counts → normal base buff).
-- ============================================================================

-- No backfill possible without H3 extension. New runs will populate the column.

-- ============================================================================
-- STEP 3: Update finalize_run to accept and store p_hex_city_parents (Res-6)
-- ============================================================================

-- Drop the current version so we can add the new parameter
DROP FUNCTION IF EXISTS public.finalize_run(
  UUID, TIMESTAMPTZ, TIMESTAMPTZ, DOUBLE PRECISION, INTEGER,
  TEXT[], INTEGER, DOUBLE PRECISION, INTEGER, INTEGER, TEXT[], TEXT
);
...
CREATE OR REPLACE FUNCTION public.finalize_run(
  ...
  p_hex_city_parents TEXT[] DEFAULT NULL   -- NEW: Res-6 district parent per hex
)
RETURNS jsonb
...
BEGIN
  ...
  IF p_hex_path IS NOT NULL AND array_length(p_hex_path, 1) > 0 THEN
    ...
    FOREACH v_hex_id IN ARRAY p_hex_path LOOP
      v_parent_hex := NULL;
      v_city_parent_hex := NULL;

      IF p_hex_parents IS NOT NULL AND v_idx <= array_length(p_hex_parents, 1) THEN
        v_parent_hex := p_hex_parents[v_idx];
      END IF;

      -- Populate district_hex (Res-6) from p_hex_city_parents if provided
      IF p_hex_city_parents IS NOT NULL AND v_idx <= array_length(p_hex_city_parents, 1) THEN
        v_city_parent_hex := p_hex_city_parents[v_idx];
      END IF;
      ...
      IF v_current_flipped_at IS NULL OR p_end_time > v_current_flipped_at THEN
        ...
        VALUES (v_hex_id, v_team, p_end_time, v_parent_hex, v_city_parent_hex)
...
-- ============================================================================
-- STEP 4: Fix calculate_daily_buffs — use district_hex column on hexes
-- ============================================================================
...
BEGIN
  ...
  LOOP
    -- FIX: Query hexes by district_hex (Res-6) column, NOT parent_hex (Res-5).
    -- Previously: WHERE h.parent_hex = v_city_hex  ← always 0 (Res-5 ≠ Res-6)
    -- Now:        WHERE h.district_hex = v_city_hex ← correct Res-6 filter
    SELECT
...
-- ============================================================================
-- STEP 5: Fix get_user_buff fallback — use district_hex column on hexes
-- ============================================================================
...
BEGIN
  ...
  ELSE
    ...
    -- FIX: Query by district_hex (Res-6) column, NOT parent_hex (Res-5).
    -- Previously: WHERE parent_hex = v_district_hex  ← always empty (Res-5 ≠ Res-6)
    -- Now:        WHERE district_hex = v_district_hex ← correct Res-6 filter
    SELECT (
      CASE v_user.team
