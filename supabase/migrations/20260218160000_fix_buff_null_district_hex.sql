-- Fix get_user_buff() early return when users.district_hex is NULL.
-- The function now accepts an optional p_district_hex parameter (from client)
-- and uses COALESCE(p_district_hex, users.district_hex) to allow the client
-- to provide the district hex when the database value hasn't been set yet.
-- This matches the pattern used by get_team_rankings().
...
BEGIN
  ...
  -- Use client-provided district_hex as fallback (matches get_team_rankings pattern)
...
-- Also update app_launch_sync to delegate to get_user_buff() instead of
-- querying daily_buff_stats directly (which used wrong column names and
-- couldn't compute elite status dynamically).
...
BEGIN
  ...
  -- Delegate to get_user_buff for consistent buff calculation
